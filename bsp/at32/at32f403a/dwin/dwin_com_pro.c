...
uint8_t Outage_flag = 0;   //断电时需要不需要赋值信息   0没有复位  1复位
...
typedef struct
{
    uint16_t  chargevol_A;			//充电A相电压 0.1v
    uint16_t  chargecur_A;			//充电A相电流 0.01a

    uint16_t  chargevol_B;			//充电B相电压 0.1v
    uint16_t  chargecur_B;			//充电B相电流 0.01a

    uint16_t  chargevol_C;			//充电B相电压 0.1v
    uint16_t  chargecur_C;			//充电B相电流 0.01a
...
_Outage_Recharg OutageRecharg;   //断电续充标志位2023年8月31日
...
CH_T			  DChargeInfo = {0};		//断电或复位 充电存储信息
...
//第一个字节表示时候已经结算     				  billflag:0表示已经结算			billflag：1表示未结算
    ...
    if(billflag != 0)
    {
        ...
        //单独存储断电交易记录
        fal_partition_erase(charge_Storage_info,0, sizeof(stChTcb.stCh));
        fal_partition_write(charge_Storage_info,0,(uint8_t*)&stChTcb.stCh,sizeof(stChTcb.stCh));  //存储充电时断电交易记录
    }
...
uint8_t  APPTransactionrecord(STOP_REASON Stop_reason,_SHOW_NUM Charg_mode,_SHOW_NUM GUNnum,uint32_t CurNum)
{
    ...
    //SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy /10;   //记录2位小数点
    SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy;   //记录3位小数点
...
**note(s)    :
**Author     :CSH
**Create_Time:2023年8月31日09:33:23
***************************************************************/
uint8_t Write_OutageRecharg_flag(uint8_t Charge_flag, uint8_t Records_Num, uint32_t NetAloneCard_blance,uint8_t *cardNUM)
{
    OutageRecharg.Chargeflag = Charge_flag;            //充电标志位     充电中0x55     未在充电中0x11
    OutageRecharg.Records_Num = Records_Num;        //结算状态标志位 存储条数
    OutageRecharg.NetAloneCard_blance = NetAloneCard_blance;  //单机或者网络卡 卡内余额
    memcpy(OutageRecharg.Card_NUM,cardNUM,sizeof(OutageRecharg.Card_NUM));  //网络卡/单机卡 赋值当前的卡号

    fal_partition_erase(Outage_Recharging_info, 0, sizeof(_Outage_Recharg));
    fal_partition_write(Outage_Recharging_info,0,(uint8_t*)&OutageRecharg,sizeof(_Outage_Recharg));
}
...
static uint8_t Period_WriterFmBill(uint32_t time)
{
    ...
    {
		//if(++count >= ((CM_TIME_5_SEC)/time) )   // 5s存储一次(test用)
        if(++count >= ((CM_TIME_5_MIN*2)/time) )   //之前为30s存一下，flash用不了多久，改成10分钟一次
        {
            count = 0;
            //写入A枪记录
...
#else
            {
                ...
                {
                    APPTransactionrecord(END_CONDITION,SHOW_START_APP,SHOW_GUNA,RECODECONTROL.RecodeCurNum);//交易记录写入(未结算)
                }
                else
                {
...
static void dwin_show_state(void)
{
    ...
    char * net = "https://api.huichongchongdian.com/cpile/coffee/";       /**HY二微码前缀**/
...
//#warning hycsh "模拟金额和电量end"
        ...
        {
            ChargeInfo.chargecur_A = stChTcb.stRn8209.usCurrent;  //赋值ABC三相的电压电流
            ChargeInfo.chargevol_A = stChTcb.stRn8209.usVolt;
            ChargeInfo.chargecur_B = stChTcb.stRn8209.usCurrentb;
            ChargeInfo.chargevol_B = stChTcb.stRn8209.usVoltb;
            ChargeInfo.chargecur_C = stChTcb.stRn8209.usCurrentc;
            ChargeInfo.chargevol_C = stChTcb.stRn8209.usVoltc;
        }
        else
        {
            //==外部电表时，三相各电压电流
            ChargeInfo.chargecur_A = dlt645_info.out_cur*100;
            ChargeInfo.chargevol_A = dlt645_info.out_vol*10;

            ChargeInfo.chargecur_B = dlt645_info.out_cur_B*100;
            ChargeInfo.chargevol_B = dlt645_info.out_vol_B*10;

            ChargeInfo.chargecur_C = dlt645_info.out_cur_C*100;
            ChargeInfo.chargevol_C = dlt645_info.out_vol_C*10;
        }
        ShowChargeInfo.chargecur_A = (ChargeInfo.chargecur_A & 0x00ff) << 8 |  (ChargeInfo.chargecur_A & 0xff00) >> 8;  //A相电压和电流
        ShowChargeInfo.chargevol_A = (ChargeInfo.chargevol_A & 0x00ff) << 8 |  (ChargeInfo.chargevol_A & 0xff00) >> 8;

        ShowChargeInfo.chargecur_B = (ChargeInfo.chargecur_B & 0x00ff) << 8 |  (ChargeInfo.chargecur_B & 0xff00) >> 8;  //B相电压和电流
        ShowChargeInfo.chargevol_B = (ChargeInfo.chargevol_B & 0x00ff) << 8 |  (ChargeInfo.chargevol_B & 0xff00) >> 8;

        ShowChargeInfo.chargecur_C = (ChargeInfo.chargecur_C & 0x00ff) << 8 |  (ChargeInfo.chargecur_C & 0xff00) >> 8;  //C相电压和电流
        ShowChargeInfo.chargevol_C = (ChargeInfo.chargevol_C & 0x00ff) << 8 |  (ChargeInfo.chargevol_C & 0xff00) >> 8;
...
//        ShowChargeInfo.chargepwer = (ChargeInfo.chargepwer & 0x00ff) << 8 |  (ChargeInfo.chargepwer & 0xff00) >> 8;
        ...
        /*3位小数点*/
        ChargeInfo.chargepwer = stChTcb.stCh.uiChargeEnergy; //充电电量
        ShowChargeInfo.chargepwer = ((ChargeInfo.chargepwer & 0x000000FF) << 24 |  (ChargeInfo.chargepwer & 0x0000FF00) << 8 | \
                                     (ChargeInfo.chargepwer & 0x00FF0000) >> 8 | (ChargeInfo.chargepwer & 0xFF000000) >> 24);

        ChargeInfo.chargemoney = stChTcb.stCh.uiAllEnergy/100;   //充电金额
        ShowChargeInfo.chargemoney = ((ChargeInfo.chargemoney & 0x000000FF) << 24 |  (ChargeInfo.chargemoney & 0x0000FF00) << 8 | \
                                      (ChargeInfo.chargemoney & 0x00FF0000) >> 8 |  (ChargeInfo.chargemoney & 0xFF000000) >> 24);
        ...
        rt_thread_mdelay(50); //延迟一下，显示数据正常
        PrintStr(DIS_ADD(HYMenu24.FrameID,0),(uint8_t*)&ShowChargeInfo,sizeof(ShowChargeInfo));
...
//        if(((DisSysConfigInfo.standaloneornet == DISP_CARD_mode) || (DisSysConfigInfo.standaloneornet == DISP_CARD)) &&((curtick - lasttick) > 5))
//        {
//            lasttick = curtick;
//            stChTcb.stCh.uiChStoptTime = gs_SysTime;	//停止充电时(重复写记录时，只写停止时间即可)
//            APPTransactionrecord(END_CONDITION,SHOW_START_CARD,SHOW_GUNA,RECODECONTROL.RecodeCurNum);//交易记录写入(未结算)
//            //单独存储断电交易记录
//            fal_partition_erase(charge_Storage_info,0, sizeof(stChTcb.stCh));
//            fal_partition_write(charge_Storage_info,0,(uint8_t*)&stChTcb.stCh,sizeof(stChTcb.stCh));  //存储充电时断电交易记录（复制存储来用）
//        }
...
static void flash_para_init(void)
{
    ...
    if(fal_partition_read(charge_Storage_info,0,(uint8_t*)&DChargeInfo,sizeof(DChargeInfo)) < 0)  //断电续充A枪的信息(0-500)
    {
        rt_kprintf("Partition read error! Flash device(%s) read error!", charge_Storage_info->flash_name);
    }
...
//    if(DisSysConfigInfo.Port != NetConfigInfo[NET_YX_SELCT].port)
    /***如果第一次烧录为空，则正常写入ip和端口，如果第二次和第一次烧录协议不一样，则修改**/
//    if((DisSysConfigInfo.Port==0xFF)||(DisSysConfigInfo.XYSelece != NetConfigInfo[NET_YX_SELCT].XYSelece))
//    {
//        //说明flash没有设置过，为初始值
//        DisSysConfigInfo.Port = NetConfigInfo[NET_YX_SELCT].port;
//        fal_partition_erase(dwin_info_part, 0, sizeof(_DIS_SYS_CONFIG_INFO));
...
**note(s)    :
**Author     :CSH
**Create_Time:2023年8月31日09:30:22
***************************************************************/
void rest_charge_dispose(void)
{
    if(fal_partition_read(Outage_Recharging_info,0,(uint8_t*)&OutageRecharg,sizeof(_Outage_Recharg)) < 0)    //读取断电前信息
    {
    ...
    if(DisSysConfigInfo.standaloneornet == DISP_NET)
    {
        ...
        if(OutageRecharg.Chargeflag == 0x55)	  //如果开始等于0x55
        {
            if(stChTcb.ucState == INSERT)  //再次启动充电
            {
                Outage_flag = 1;     // 在充电电量中赋值信息的标志位
                APP_SetResendBillState(GUN_A,0);  //设置暂不发送订单
                send_ch_ctl_msg(1,0,4,0);
                RECODECONTROL.RecodeCurNum = OutageRecharg.Records_Num;//存储写入的条数
                if(OutageRecharg.Card_NUM[0] !=0 && OutageRecharg.Card_NUM[1] !=0 && OutageRecharg.Card_NUM[2] !=0)
                {
                    _4G_SetStartType(GUN_A,_4G_APP_CARD);		//前三个字节都不为0  设置net卡启动 (默认是app启动)
                    memcpy(m1_card_info.uidByte,OutageRecharg.Card_NUM,sizeof(m1_card_info.uidByte));   //断电前卡号
                }else
				{
				   _4G_SetStartType(GUN_A,_4G_APP_START); 
				}
#if(NET_YX_SELCT == XY_HY)
                _HY_RestUpdataData(GUN_A);  //HY断电赋值数据
#endif
#if(NET_YX_SELCT == XY_YKC)     //YKC赋值信息
                _YKC_RestUpdataData(GUN_A);  //HY断电赋值数据
                Balance[GUN_A] = OutageRecharg.NetAloneCard_blance;
#endif
                return;
            }
            else
            {
                Write_OutageRecharg_flag(0x11,0,0,ZERO);   //清空结束充电状态标志位（连上网后，自动发送订单）
            }
        }
    }
    else   //单机
    {
        if(OutageRecharg.Chargeflag == 0x55)  //断电续充标志位
        {
            if(stChTcb.ucState == INSERT)  //再次启动充电
            {
                send_ch_ctl_msg(1,0,4,0);
                Outage_flag = 1;    //赋值信息的标志位（包含）
                M1Control.m1_if_balance = 1;
                RECODECONTROL.RecodeCurNum = OutageRecharg.Records_Num;//存储写入的条数
                memcpy(m1_card_info.uidByte,OutageRecharg.Card_NUM,sizeof(m1_card_info.uidByte));   //断电前卡号
                memcpy(&m1_card_info.balance,&OutageRecharg.NetAloneCard_blance,sizeof(OutageRecharg.NetAloneCard_blance));   //断电前卡内余额
                return;
            }
            else
            {
                Write_OutageRecharg_flag(0x11,0,0,ZERO);  //没有插枪 写结束充电状态标志位
            }
        }
    }
}
            ...
            case CM_CHTASK_MODULE_ID:   /* 充电任务模块ID */
                ...
                case CH_TO_DIP_STARTSUCCESS: 	//启动成功
                    if(DisSysConfigInfo.standaloneornet == DISP_NET)
                    {
                        ...
                        if(_4G_GetStartType(GUN_A) == _4G_APP_CARD)   //判断一下是net卡 /app启动
                        {
#if(NET_YX_SELCT == XY_HY)
                            {
                                Write_OutageRecharg_flag(0x55,RECODECONTROL.RecodeCurNum,0,m1_card_info.uidByte);   //net卡 写继续充电状态标志位
                            }
#elif(NET_YX_SELCT == XY_YKC)  //net卡 写继续充电状态标志位
                            {
                                Write_OutageRecharg_flag(0x55,RECODECONTROL.RecodeCurNum,Balance[GUN_A],m1_card_info.uidByte);
                            }
#else
                            {
                                Write_OutageRecharg_flag(0x55,RECODECONTROL.RecodeCurNum,0,0);
                            }
#endif
                        }
                        else
                        {
#if(NET_YX_SELCT == XY_YKC)  //app启动 写继续充电状态标志位
                            Write_OutageRecharg_flag(0x55,RECODECONTROL.RecodeCurNum,Balance[GUN_A],0);  //YKC需要写一下金额
#elif(NET_YX_SELCT == XY_HY)
                            Write_OutageRecharg_flag(0x55,RECODECONTROL.RecodeCurNum,0,0);  //HY卡号
#endif
                        }
                ...
                case CH_TO_DIP_STARTFAIL:    	//启动失败
                    ...
                    //跳转到刷卡结算界面
                    Write_OutageRecharg_flag(0x11,0,0,ZERO);  //清空续充充电状态标志位
                    break;
