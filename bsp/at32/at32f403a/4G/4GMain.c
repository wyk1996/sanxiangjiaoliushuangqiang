...
void Task4GMain(void *pdata)
{
    ...
    while(1)
    {
        ...
        if(APP_GetSIM7600Mode() == MODE_DATA)
        {
			...
			nowSysTime = rt_tick_get(); 	//10分钟没连上服务器，而且没在充电中，就重启设备
			...
			{
//				static uint32_t lasttttime = 0 ;
//				if(lasttttime == 0)
//				{
//				 lasttttime = rt_tick_get();
//				}
//				if((rt_tick_get() - lasttttime) > CM_TIME_10_SEC)
//				{
//				 	APP_SetSIM7600Mode(MODE_HTTP); //连上网后，开始升级
//				}
        ...
        else
        {
            ...
            {
                //memcpy(HttpInfo.ServerAdd,"http://hy.shuokeren.com/uploads/33111.bin",strlen("http://hy.shuokeren.com/uploads/23999.bin"));
			...
			{
				printf("shibai  severconnect shibai");
				nvic_system_reset(); //升级失败，系统复位（自动上传当前的版本号）
			}
        }
        rt_thread_mdelay(1000);
